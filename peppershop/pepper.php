...
class pepper {
    ...
    var $tableerp  = array("partnumber"=>"artikel_nr","description"=>"name","notes"=>"beschreibung",
                           "unit"=>"anzahl_einheit","weight"=>"gewicht","sellprice"=>"preis","stdprice"=>"stdpreis",
                           "tax"=>"mwst_satz","image"=>"bild_gross","onhand"=>"lagerbestand",'staffel'=>'staffel');
    var $tableshop = array("datum"=>"transdate","rechnungsbetrag"=>"amount","nettobetrag"=>"netamount","waehrung"=>"curr",
                           ...
                           "kreditkarte"=>"kreditkarte","versandart"=>"shipvia","shoppreis"=>"shoppreis");
    var $custshop   = array("kontakt"=>"contact","ort"=>"city","plz"=>"zipcode","land"=>"country","tel"=>"phone",
                            ...
                            "anrede"=>"greeting","kunden_id"=>"shopid","kunden_nr"=>"kunden_nr",
    ...
    var $translate  = array("description","partsgroup","longdescription","image","unit");
    var $pic = false;

    function pepper($db,$error,$dbname,
                    $divStd,$divVerm,$minder,$nachn,$versandS,$versandV,$paypal,$treuhand,
                    $mwstLX,$mwstPGLX,$pricegroup,$mwstS,$variantnr,$variant,$pic=false,$nopic=false,$nopicerr=false,$nofiles=false,
        ...
        $this->mwstLX   = $mwstLX;
        $this->mwstPGLX = $mwstPGLX;
        $this->pricegr  = $pricegroup;
        $this->mwstS    = $mwstS;
        $this->VariantNr = ($variantnr==1)?true:false;
        $this->variant   = ($variant==1)?true:false;
        $this->pic       = $pic;
        ...
        echo "!ERP: ".$this->erpcode."!Shop: ".$this->shopcode."!<br>";
    }
    function _toERP($txt) {
    ...
    function saveArtikel($data,$lang,$lastpartnr) {
         $langID = $this->getLang(strtolower($lang));
         $values = $this->translateTable($data,"tableerp"); 
         if ( $values['preis'] > 0 ) {  //Preisgruppe hat Preis
             if ($this->mwstPGLX and !$this->mwstS) { //ERP-Bruttopreis Shop-Nettopreis
                  $values['preis'] = round($values['preis'] / (1 + $values["mwst_satz"]),2);
             } else if (!$this->mwstPGLX and $this->mwstS) { //ERP-Nettopreis Shop-Bruttopreis
                  $values['preis'] = round($values['preis'] * (1 + $values["mwst_satz"]),2);
             }
         } else {
             if ($this->mwstLX and !$this->mwstS) { //ERP-Bruttopreis Shop-Nettopreis
                  $values['preis'] = round($values['stdpreis'] / (1 + $values["mwst_satz"]),2);
         ...
         if ( strpos($values['artikel_nr'],'-') > 0 ) {
            $tmp = explode('-', $values['artikel_nr']);
            $txt = explode(';',$values['name']);
            $values["artikel_id"] = $this->checkArtikelOK($tmp[0]);
            $this->error->write('nach chkA',$values["artikel_id"]);
            $this->error->write('nach chkA',print_r($tmp,true));
            if ( $values["artikel_id"] < 1 )  {
                $values['artikel_nr'] = $tmp[0];
                $values['name'] = $txt[0];
                $preis = $values['preis'];  //Preis merken
                $values['preis'] = -1;      //Nur Varianten, dann Preis -1
                $values["artikel_id"] = $this->insertArtikel($values);
                if ($values["artikel_id"]<1) {
                    return $values["artikel_id"];
                } else {
                    //Wieso das? Wenn nur Varianten, dann Grundartikel = -1!
                    //$sql = "UPDATE artikel SET preis = $preis WHERE artikel_id = ".$values["artikel_id"];
                    //$rc  = $this->db->query($sql);
                    //Die Variation hat dann den Preis. 
                    $values['preis'] = $preis;
                }
            };
            if ( $tmp[1] != '' ) {
                if( $txt[1] == '' ) {
                    $values['variant'] = $txt[0]." - ".$tmp[1];
                } else {
                    $values['variant'] = $txt[1];
                };
                $this->insVariant($values,$tmp[1]);
            } else {
                return -1;
            }
         } else {
    ...
    function insVariant($data,$nr) {
        $sql = "SELECT max(variations_nr) as max FROM artikel_variationen WHERE FK_Artikel_ID = ".$data['artikel_id']." ORDER by variations_id";
        $rs  = $this->db->getOne($sql);
        $this->error->write('insVar',print_r($rs,true));;
        if ( count($rs) > 0 ) {
           $this->error->write('lastnr',$rs['max']);;
           if ( $nr <= $rs['max'] ) {
               $sql = "UPDATE artikel_variationen SET variationstext = '%s', aufpreis = %f WHERE variations_nr = %d and fk_artikel_id = %d";
               $rc  = $this->db->query(sprintf($sql,$data['variant'],$data['preis'],$nr,$data['artikel_id']));
            //update
            } else if ( $rs['max']+1 == $nr ) {
                $sql = "INSERT INTO artikel_variationen (variations_nr,fk_artikel_id,variationstext,aufpreis) VALUES (%d,%d,'%s',%f)";
                $rc  = $this->db->query(sprintf($sql,$nr,$data['artikel_id'],$data['variant'],$data['preis']));
            //insert
            }
        } else {
            //insert
            $sql = "INSERT INTO artikel_variationen (variations_nr,fk_artikel_id,variationstext,aufpreis) VALUES (1,%d,'%s',%f)";
            $this->error->write('ins',sprintf($sql,$data['artikel_id'],$data['variant'],$data['preis']));;
            $rc  = $this->db->query(sprintf($sql,$data['artikel_id'],$data['variant'],$data['preis']));
        };
        return $rc;
    }
    ...
    function insertArtikel($data) {
        ...
        if ($rs['name'] == $newID) {
            ...
            if ( !$rc ) {
                $sql = "DELETE FROM artikel WHERE name = '".$newID."'";
                $rc = $this->db->query($sql);
            }
        } else { return false; }
        return $rc;
    ...
    function updateArtikel($values) {
        ...
        if ( !$this->nofiles ) {
             ...
             if ( $values['bild_gross'] != '' ) {
                 ...
                 if ( count($tmp) == 3 ) {
                     if ( $this->pic &&  $this->pic->copyImage($values['artikel_nr'],$values['bild_gross'],$tmp[2]) ) {
                         $sql .= "bild_gross = :bild_gross, bild_klein = :bild_klein, bildtyp = :bildtyp, ";
                         $values['bild_gross'] = $values['artikel_nr']."_gr.".$tmp[2];
                         $values['bild_klein'] = $values['artikel_nr']."_kl.".$tmp[2];
                         $values['bildtyp'] = $tmp[2];
                     } else if ( $this->nopic && $this->nopicerr ){
                         $this->pic->copyImage('nopic',$this->nopic.".".$this->nopicext,$this->nopicext); 
                         $sql .= "bild_gross = :bild_gross, bild_klein = :bild_klein, bildtyp = :bildtyp, ";
                         $values['bild_gross'] = $this->nopic."_gr.".$this->nopicext;
                         $values['bild_klein'] = $this->nopic."_kl.".$this->nopicext;
                         $values['bildtyp'] = 'jpg';
                     } 
        ...
        $nr = 1;
        if ( $values['staffel'] != '' ) {
            $staffeln = split('!',$values['staffel']);
            if ( $staffeln ) foreach ($staffeln as $element) {
                $preis = split(':',$element);
                $sql .= 'staffel_ab_'.$nr.' = :staffel_ab_'.$nr.',';
                $sql .= 'staffel_preis_'.$nr.' = :staffel_preis_'.$nr.',';
                $values['staffel_ab_'.$nr] = $preis[0];
                $values['staffel_preis_'.$nr] = str_replace(',','.',$preis[1]);
                $nr ++;
            }
        }
        while ( $nr < 10 ) { // nicht gesetzt Staffeln löschen
            $sql .= 'staffel_ab_'.$nr.' = :staffel_ab_'.$nr.',';
            $sql .= 'staffel_preis_'.$nr.' = :staffel_preis_'.$nr.',';
            $values['staffel_ab_'.$nr] = 0;
            $values['staffel_preis_'.$nr] = 0;
            $nr ++;
        }
        unset($values['staffel']);
        $sql .= "mwst_satz = :mwst_satz, anzahl_einheit = :anzahl_einheit ";
        if ( $this->variant AND $values['parent'] != '' ) $sql .= ",parent_nr = :parent ";
        ...
        foreach ($rs as $row) {
             ...
             if ($row["versandkosten"]>0) {
                 if ($versandsteuer ==  $this->versandV['TAX']) {
                     $artikel[]  = array("partnumber"=>$this->versandV['NR'],"description"=>$this->versandV['TXT'],"taxrate"=>$this->versandV['TAX']*100,
                                          "qty"=>1,"unit"=>$this->versandV['Unit'],"sellprice"=>$row["versandkosten"]);
                 } else {
                     $artikel[]  = array("partnumber"=>$this->versandS['NR'],"description"=>$this->versandS['TXT'],"taxrate"=>$this->versandS['TAX']*100,
                                          "qty"=>1,"unit"=>$this->versandS['Unit'],"sellprice"=>$row["versandkosten"]);
                 }
             }
             if ($row["nachnamebetrag"]>0) 
                 $artikel[] = array("partnumber"=>$this->nachn['NR'],"description"=>$this->nachn['TXT'],"taxrate"=>$this->nachn['TAX']*100,
                                    "qty"=>1,"unit"=>$this->nachn['Unit'],"sellprice"=>$row["nachnamebetrag"]);
             if ($row["paypalkosten"]>0) 
                 $artikel[]   = array("partnumber"=>$this->paypal['NR'],"description"=>$this->paypal['TXT'],"taxrate"=>$this->paypal['TAX']*100,
                                      "qty"=>1,"unit"=>$this->paypal['Unit'],"sellprice"=>round($row["paypalkosten"],2));
             if ($row["treuhandkosten"]>0) 
                 $artikel[] = array("partnumber"=>$this->treuhand['NR'],"description"=>$this->treuhand['TXT'],"taxrate"=>$this->treuhand['TAX']*100,
                                      "qty"=>1,"unit"=>$this->treuhand['Unit'],"sellprice"=>$row["treuhandkosten"]);
             if ($row["mindermengenzuschlag"]>0) 
                 $artikel[] = array("partnumber"=>$this->minder['NR'],"description"=>$this->minder['TXT'],"taxrate"=>$this->minder['TAX']*100,
                                    "qty"=>1,"unit"=>$this->minder['Unit'],"sellprice"=>$row["mindermengenzuschlag"]);
             if ($row["versandland_id"] == "DE") {
                 $taxzone_id = 4;
             } else if (in_array($row["versandland_id"],$this->EU)) {
    ...
    function getBestellungKunde($bestellung) {
        ...
        $rs=$this->db->getOne($sql);
        if ($rs["firma"]) {
            $rs["kontakt"] = $this->_toERP($rs['anrede'].' '.$rs["vorname"]." ".$rs["nachname"]);
        ...
        if ($rs) {
            ...
            $this->error->write("pepper","Der Kunde der Bestellung $bestellung konnte nicht gelesen werden");
            return false;
        }
    }
    function getBestellArtikel($bestellung) {
        ...
        $sql .= "(SELECT fk_kategorie_id FROM artikel_kategorie WHERE fk_artikel_id=P.artikel_id LIMIT 1) as partsgroup ";
    ...
    function setKundenNr($id,$nr) {
        $sql = "UPDATE kunde SET kunden_nr = '$nr' WHERE kunden_id = $id";
        $rc = $this->db->query($sql);
        if (!$rc) {
    ...
    function getAllArtikel($Varianten=False) {
        if (!$this->Kategorien) $this->getKategorien();
        if ($Varianten) {
            $sql  = "SELECT CONCAT( a.artikel_nr,  '-', v.variations_nr ) AS nr, a. * , v.variations_nr, v.variationstext,v.aufpreis, ";
            $sql .= "(SELECT fk_kategorie_id FROM artikel_kategorie WHERE fk_artikel_id=a.artikel_id LIMIT 1) as katid ";
            $sql .= "FROM artikel a ";
            $sql .= "LEFT JOIN artikel_variationen v ON a.artikel_id = v.fk_artikel_id ";
            $sql .= "WHERE (v.Variations_Grp =1 OR v.Variations_Grp IS NULL)";
            // Es wird nur die erste Variantengruppe angelegt.
        } else {
            $sql  = "SELECT a.*,(SELECT fk_kategorie_id FROM artikel_kategorie WHERE fk_artikel_id=a.artikel_id LIMIT 1) as katid ";
            $sql .= "FROM artikel a ";
        }
        $rs = $this->db->getAll($sql);
        if ($rs) foreach ($rs as $row) {
            $row['partsgroup'] = $this->_toERP($this->Kategorien[$row['katid']]);
            if ($Varianten AND $row['nr']) {
                $row['name'] = $this->_toERP($row['name'].', '.$row['variationstext']);
                $row['artikel_nr'] = $row['nr'];
                $row['preis'] += $row['aufpreis'];
            } else {
            ...
            $row['anzahl_einheit'] = $this->_toERP($row['anzahl_einheit']);
            if ( $this->pricegr > 0 ) {
                if ( !($this->mwstPGLX == $this->mwstS) ) { // MwSt nur berücksichtigen wenn ungleich gesetzt
                    if ( !$this->mwstPGLX ) {
                        $row['shoppreis'] = round(($row['shoppreis'] / (100 + $row['mwst_satz']) * 100),$this->dezimal);
                    } else {
                        $row['shoppreis'] = round(($row['shoppreis'] / 100 * (100 + $row['mwst_satz'])),$this->dezimal);
                    }
		        }
            }
            if ( !($this->mwstLX == $this->mwstS) ) { // MwSt nur berücksichtigen wenn ungleich gesetzt
                if ( $this->mwstS ) {
                    $row['preis'] = round(($row['preis'] / (100 + $row['mwst_satz']) * 100),$this->dezimal);
                } else {
                    $row['preis'] = round(($row['preis'] / 100 * (100 + $row['mwst_satz'])),$this->dezimal);
                }
		    }
            $data[] = $this->translateTable($row,"orderparts");
        }
