...
!!!!! Kivi > 3.0.0 oder ok !!!!! currency_id

Multishop: Hierfür müssen benutzerdefinierte Variablen für Artikel angelegt werden.
...
class erp {
    ...
    function getParts($stdprice=0,$shop=0,$start,$max=0) {
        ...
        if ($stdprice>0) {
             ...
             if ( $this->parent != '' ) $sql .= ','.$this->parent.' as parent';
             if ($this->lager>1) {
        ...
        if ($shop>0) {  
            ...
            $where .= "AND (CV.config_id = (SELECT id FROM custom_variable_configs WHERE name = 'shop$shop' and module = 'IC') AND CV.bool_value = 't')";
        }
        $where .= "AND shop = 't' ";
        ...
        if ($rs) for($i = 0; $i < count($rs); $i++) {
           $rs[$i]['tax'] = $this->TAX[4][$rs[$i]['bugru']]['rate']; //Immer 4???
        }
        return $rs;
    }
    ...
    function insCustomer($data) {
        ...
        if ($data['customernumber']>0) {
            ...
            $sql = "SELECT id FROM customer WHERE customernumber = '".$data['customernumber']."'";
            $rs = $this->db->getOne($sql);
            $this->error->out($data['customernumber']);
            if ( isset($rs['id']) ) { // Insert - Select war ok.
                $data['customer_id'] = $rs['id'];
                $this->insCustCVar($rs['id'],$data['shopid']);
            } else {
    ...
    function insCustCVar($id,$shop) {
        if ( $this->cvarid > 0 ) {
            $sql = "INSERT INTO custom_variables (config_id,trans_id,text_value) VALUES (%d,%d,'%s')";
            $cvar = $this->db->query(sprintf($sql,$this->cvarid,$id,$shop));
            if ( $cvar ) {
                $this->error->out($id." CV");
            } else {
                $this->error->out($id." CVar-Fehler");
            }
        } else {
            $this->error->out(" noCV");
        }
    ...
    function chkCustomer(&$data) {
        $this->error->write('chkCust',print_r($data,true));
        $cvar = false;
        ...
        if ( !isset($rs['id']) AND $data['kunden_nr'] != '' ) { //CVar ist nicht gesetzt, vlt. die Kundennummer bekannt?
            $sql = "SELECT * FROM customer WHERE customernumber = '".$data['kunden_nr']."'";
            $rs = $this->db->getOne(sprintf($sql,$data['kunden_nr']));
            $cvar = true;
        }
        if ( isset($rs['id']) ) {  //Kunde bekannt
            $data['customer_id'] = $rs['id'];
            $data['kunden_nr']   = $rs['customernumber'];
            $this->error->out('Update Kunde: '.$data['name'].' '.$data['customer_id'].' '.$data['kunden_nr'].' ');
            ...
            if ( $cvar ) $this->insCustCVar($data['customer_id'],$data['shopid']);
            if ( !isset($rc->rownum) ) $data['customer_id'] = -1;
        } else {  // Neukunde
            $rc = $this->insCustomer($data);
            $data['customer_id'] = $rc['customer_id'];
            $data['kunden_nr'] = $rc['customernumber'];
        }
        $this->error->out('',true);
    }
    function mkAuftrag($data,$shop,$longtxt) {
        ...
        $this->chkCustomer($data['customer']);
        if ( $data['customer']['customer_id'] <= 0 ) {
             $this->error->write('erplib','Insert/Update Kundendaten');
             $this->error->out(" Kundendaten ");
             return -1;
        }
        $parts = $data['parts'];
        $data['customer_id'] = $data['customer']['customer_id'];
        $data["kunden_nr"]   = $data['customer']['kunden_nr'];
        unset($data['parts']);
        unset($data['customer']);
        if ($this->doordnr == 1) {
    ...
    function chkPartnumber($data,$new=True,$long=false,$mknew=false) {
       ...
       //$this->error->write('chkPartnumber',print_r($rs,true));    
       //$this->error->write('chkPartnumber',$rs==true);    
       //$this->error->write('chkPartnumber',"mknew:$mknew!new:$new!mkPart:".$this->mkPart);    
       if ($rs) {
    ...
    function mkNewPart($data) {
       ...
       */
       ...
       //$this->error->write('mkNewPart',print_r($data,true));           
