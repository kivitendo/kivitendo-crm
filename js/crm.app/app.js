...
function crmInitFormEx( crmFormModel, table, max_rows = 0, container = null){
    ...
    for( let i = 0; i < max_rows; i++ ){
        ...
        let addItem = function( item ){
            ...
            if( item.type == 'part' ) tabledata += '<td' +  ( ( exists(item.class) )? ' class="' + item.class + '" ' : '' ) + '>' + kivi.t8( item.label ) + '</td><td' +  ( ( exists(item.class) )? ' class="' + item.class + '" ' : '' ) + '><input type="hidden" id="' + item.name + '"></input><input type="text" id="' + item.name + '-label" name="'+ item.name + '" size="' + item.size + '" title="' + kivi.t8( item.tooltip ) + ' " class="crm-ignore-field"></input>';
