...
echo '
    <script>
        ...
        $(function() {
            $("#ac0").catcomplete({
                source: "jqhelp/autocompletion.php?case=name",
                minLength: '.$_SESSION['feature_ac_minlength'].',//<read> hier für wird doch gar kein php mehr benötigt! besser wäre >> minLength: kivi.myconfig.ac_minLength </read>
                delay: '.$_SESSION['feature_ac_delay'].',        //<read> dazu müssten die ac_minLength jedoch in der kivitendo_auth.user_confi-Tabelle stehen </read>
                disabled:'.($_SESSION['feature_ac']?'false':'true').',
                select: function(e,ui) {
                    showD(ui.item.src,ui.item.id, number);
                }
...
echo '
    ...
    <div id="tabs">
        ...
        <div id="tab-1">
            ...
            <form name="suche" id="suche" action="" method="get">
                <input type="text" name="swort" size="25" id="ac0" autocomplete="off">
                <button id="adress"> '.translate('.:adress:.','firma').'</button>    /* <read> die Übersetzung kann doch besser mit jquery bzw js realisiert werden </read> */
                <button id="kontakt">'.translate('.:contact history:.','firma').'</button> <br>
                <span class="liste">'.translate('.:search keyword:.','firma').'</span>
            </form>
            <div id="drop">
