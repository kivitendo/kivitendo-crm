<?php // $Id: PDF_Label.php,v 1.2 2005/09/16 11:59:28 hli Exp $
//http://www.fpdf.org/en/script/dl.php?id=29&f=tgz
////////////////////////////////////////////////////
// PDF_Label
//
...
// 1.4  : + : Added each line in an address label has an owen fontsize 
////////////////////////////////////////////////////
...
class PDF_Label extends FPDF {
    ...
    function _Get_Height_Chars($pt) {
        ...
        $_Table_Hauteur_Chars = array(6=>3, 7=>3.2, 8=>3.5, 9=>3.8, 10=>4, 11=>5, 12=>6, 13=>7, 14=>8, 15=>9,16=>10);
    ...
    function Add_PDF_Label2($texte) {
        // We are in a new page, then we must add a page
        if (($this->_COUNTX ==0) and ($this->_COUNTY==0)) {
            $this->AddPage();
        }
	$this->Set_Font_Size($texte[0]["font"]);
        $_PosX = $this->_Margin_Left+($this->_COUNTX*($this->_Width+$this->_X_Space))+3;
        $_PosY = $this->_Margin_Top+($this->_COUNTY*($this->_Height+$this->_Y_Space))+3-$this->_Line_Height;
	foreach($texte as $row){
		$this->Set_Font_Size($row["font"]);
		$_PosY+=$this->_Line_Height;
		$this->Text($_PosX,$_PosY,$row["text"]);
	}
        $this->_COUNTY++;
